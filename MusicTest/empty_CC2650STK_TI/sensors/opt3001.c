...
double opt3001_get_data(I2C_Handle *i2c) {
	...
	if (opt3001_get_status(i2c) & OPT3001_DATA_READY) {
		...
		if (I2C_transfer(*i2c, &i2cMessage)) {
		    ...
		    uint16_t val = (uint16_t)(rxBuffer[0] << 8) | rxBuffer[1];
		    uint16_t maskiE = 0b0000000000001111;
		    uint16_t maskiR = 0b1111111111110000;

		    uint16_t valE = val & maskiE;
		    uint16_t valR = val & maskiR;
		    valR = valR >> 4;
		    lux = 0.01 * pow(2, valE) * valR;
